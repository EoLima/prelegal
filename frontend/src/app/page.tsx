...
export default function Page() {
  ...
  const downloadPdf = useCallback(async () => {
    ...
    try {
      ...
      addLine(`This Mutual Non-Disclosure Agreement (the "MNDA") consists of: (1) this Cover Page and (2) the Common Paper Mutual NDA Standard Terms Version 1.0 ("Standard Terms"). Any modifications of the Standard Terms should be made on the Cover Page, which will control over conflicts with the Standard Terms.`)
      addEmptyLine()
      ...
      const mndaTerm = d.mndaTermType === 'expires'
        ? `Expires ${d.mndaTermYears} year(s) from Effective Date.`
        : 'Continues until terminated in accordance with the terms of the MNDA.'
      const confidentialityTerm = d.confidentialityType === 'years'
        ? `${d.confidentialityYears} year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.`
        : 'In perpetuity.'

      addLine('MNDA Term:', 'bold')
      addLine(`  [${d.mndaTermType === 'expires' ? 'X' : ' '}] ${mndaTerm}`)
      addLine(`  [${d.mndaTermType === 'continues' ? 'X' : ' '}] Continues until terminated in accordance with the terms of the MNDA.`)
      addEmptyLine(4)
      ...
      addEmptyLine()
      addLine('Common Paper Mutual Non-Disclosure Agreement Version 1.0 - Free to use under CC BY 4.0.', 'italic', 9)
