...
export function fillGenericTemplate(fields: Record<string, unknown>): string {
  const hr = '─'.repeat(72)
  const labelPad = Math.max(...Object.keys(fields).map((k) => k.length), 20)

  const rows = Object.entries(fields)
    .filter(([, v]) => v != null && v !== '')
    .map(([k, v]) => `${k.padEnd(labelPad)} ${v}`)
...
This document was generated by Prelegal AI based on the information you provided.
Please review carefully and consult with a legal professional before signing.`
}
